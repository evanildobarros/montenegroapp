...
class CheckboxWidget extends BasicWidget
{
    ...
    public function render(array $data, ContextInterface $context): string
    {
        ...
        if (empty($data['templateVars'][1])) {
            $data['templateVars'][1] = 'Sim';
        }
        if (empty($data['templateVars'][0])) {
            $data['templateVars'][0] = 'Não';
        }
        if ($this->_isChecked($data)) {
            $data['checked'] = true;
            $data['templateVars']['text'] = $data['templateVars'][1];
            $data['templateVars']['textClass'] = 'text-success';
        } else {
            $data['templateVars']['text'] = $data['templateVars'][0];
            $data['templateVars']['textClass'] = 'text-danger';
        }
        unset($data['val']);

        $data['onchange'] = 'checkbox($(this));';
